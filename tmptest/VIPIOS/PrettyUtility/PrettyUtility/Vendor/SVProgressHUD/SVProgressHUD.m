...
+ (void) showWithOnlyStatus:(NSString *)status
{
    [self showWithOnlyStatus:status duration:2.0f];
}

+ (void)showWithOnlyStatus:(NSString*)status duration:(NSTimeInterval)duration
{
    [SVProgressHUD show];
    [[SVProgressHUD sharedView] showImage:nil status:status duration:duration];
}
...
+ (void)showImageWithShortTime:(UIImage *)image status:(NSString *)string {
...
- (void)setStatus:(NSString *)string {
    ...
    CGFloat imageHeight = !flag ? 34.0f : 0.0f ;
    CGFloat imageY = !flag ? 44.0f : 0.0f ;
    CGRect labelRect = CGRectZero;
    
    if(string) {
        CGSize stringSize = [string sizeWithFont:self.stringLabel.font constrainedToSize:CGSizeMake(240, 300)];
        stringWidth = stringSize.width;
        stringHeight = stringSize.height;
        hudHeight = 80 + stringHeight - imageHeight;
        
        if(stringWidth > hudWidth)
            hudWidth = ceil(stringWidth/2)*2;
        
        if(hudHeight > 100) {
            labelRect = CGRectMake(12, 66-imageY, hudWidth, stringHeight);
            hudWidth+=24;
        } else {
            hudWidth+=24;  
            labelRect = CGRectMake(0, 66-imageY, hudWidth, stringHeight);
        }
    }
