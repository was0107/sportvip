...
- (void)_renderPages:(CGContextRef)context rect:(CGRect)rect
{
	...
	for (NSUInteger i = 0; i < _numberOfPages; i++) {
		...
		if (i == _displayedPage) {
			...
			image = [_currentPageImages valueForKey:[NSString stringWithFormat:@"%d",i]];
			if (nil == image) {
			...
			image = [_pageImages valueForKey:[NSString stringWithFormat:@"%d",i]];
			if (nil == image) {
...
- (void)_setImage:(UIImage *)image forPage:(NSInteger)pageIndex current:(BOOL)current
{
    ...
    [dictionary setObject:image forKey:[NSString stringWithFormat:@"%d",pageIndex]];
...
- (UIImage *)_imageForPage:(NSInteger)pageIndex current:(BOOL)current
{
    ...
    return [dictionary valueForKey:[NSString stringWithFormat:@"%d",pageIndex]];
