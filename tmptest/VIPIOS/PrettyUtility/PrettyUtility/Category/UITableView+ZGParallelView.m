...
- (void)addParallelViewWithUIView:(UIView *)aViewToAdd withDisplayRadio:(CGFloat)aDisplayRadio cutOffAtMax:(BOOL)cutOffAtMax{
    ...
    self.embededScrollView = [[[ZGScrollView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width+145, self.viewHeight)] autorelease];
    self.embededScrollView.tableView = self;
    [self.embededScrollView addSubview:aViewToAdd];
    aViewToAdd.frame = CGRectOffset(aViewToAdd.frame, -70, self.viewHeight*(1.f - self.displayRadio)/2.f);
