...
{
    [super viewDidLoad];
    [self setTitleContent:NSLocalizedString(@"SIGN IN","SIGN IN")];
...
//    [self.view addSubview:self.newButton];
    ...
    [[[self showRight] rightButton] setTitle:NSLocalizedString(@"Join","Join") forState:UIControlStateNormal];
...
- (PubTextField *)phoneTextField
{
    if (!_phoneTextField) {
        ...
        _phoneTextField = [[PubTextField alloc] initWithFrame:CGRectMake(0, 10 + kImageStartAt, 320, 40)
...
- (PubTextField *)pwdTextField
{
    if (!_pwdTextField) {
        __unsafe_unretained LoginViewController *safeSelf = self;
        _pwdTextField = [[PubTextField alloc] initWithFrame:CGRectMake(0, 51 + kImageStartAt, 320, 40)
...
- (UIButton *)forgetButton
{
    if (!_forgetButton) {
        ...
        [_forgetButton setTitle:NSLocalizedString(@"Forget Password?",@"Forget Password?")forState:UIControlStateNormal];
...
- (UIButton *)newButton
{
    if (!_newButton) {
        ...
        [_newButton setTitle:NSLocalizedString(@"New user? Join Now>",@"New user? Join Now>") forState:UIControlStateNormal];
...
- (void)requestServerForLogin:(id)sender
{
    ...
    [SVProgressHUD showWithStatus:NSLocalizedString(@"Logining...",@"Logining...")];
    ...
    idBlock errBlock = ^(id content){
        DEBUGLOG(@"failed content %@", content);
        [SVProgressHUD showErrorWithStatus:NSLocalizedString(@"Login Failed",@"Login Failed")];
        [safeSelf.confirmButton setEnabled:YES];
    };
