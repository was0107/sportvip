...
{
    NSAssert(value && [value isEqualToString:@""] , @"checkCreditCardNumber: cardNum is nil.");
    ...
    for (NSInteger i = len - 1; i >= 0; i--) {
        ...
        NSInteger num = [value substringWithRange:NSMakeRange(i, 1)].integerValue;
        if (isOdd) {//奇数位
            sumNumOdd += num;
        }else{//偶数位
            num = num * 2;
...
{
    const char *cvalue = [password UTF8String];
    int len = strlen(cvalue);
    if (len < 6 || len > 16) {
        return FALSE;
    }
...
+ (BOOL)isValid:(IdentifierType) type value:(NSString*) value
{
    ...
    switch (type)
    {
        ...
        case IdentifierTypeBirthday:
            ...
            break;
        case IdentifierTypePassword:
            result = [IdentifierValidator isValidPassword:value];
            break;
        default:
        break;
    }
