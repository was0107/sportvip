...
- (void) swipGesture:(UIGestureRecognizer *)recognizer
{
    if (self.level > 0) {
        [self back];
    }
...
- (void)recyleTableViews:(BOOL)push
{
    ...
    if (push) {
        if (_level >= 3) {
            recyleTableView = [self tableViewWithLevel:_level - 3];
        }
    }
    else {
...
- (void)applyNextAnimation:(InfiniteTreeTableView*)tableView currentLevel:(NSInteger)currentLevel
{
    ...
    InfiniteTreeTableView *currentTableView0 = nil;//
    if (currentLevel-1 >= 0) {
        currentTableView0 = [self tableViewWithLevel:currentLevel-1];
    }
    if (_animated) {
        [UIView animateWithDuration:0.3 animations:^{
            if (currentTableView0) {
                currentTableView0.left = 0;
                currentTableView.left = 40;
                frame.origin.x = 120;
...
- (void)setSeletedIndexForLevel:(NSInteger)level indexPath:(NSIndexPath*)indexPath
{
    ...
    InfiniteTreeTableView *treeTableView1 = [self tableViewWithLevel:level+2];
    if (treeTableView1) {
        [self back];
    }
